...
""".format(
    ...
    def add_attribution_file(self, text=None):
        """
        Adds an attribution/license notice file to the map.
        """
        if text is None:
            text = self.__downloader.manifest().get("attribution")

        if isinstance(text, list):
            text = "\n".join(str(item) for item in text)
        elif isinstance(text, dict):
            text = "\n".join(
                "{}: {}".format(key, value) for key, value in sorted(text.items())
            )

        if not text:
            text = """Data attribution

This map was generated by XCSoar Map Generator.
Check the source data repository manifest for exact data-source notices.
"""

        if not text.endswith("\n"):
            text += "\n"

        dst = os.path.join(self.__dir_temp, "ATTRIBUTION.txt")
        spew(dst, text)
        self.__files.add(dst, True)
